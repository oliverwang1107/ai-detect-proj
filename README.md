...
這是一個的 AI 生成圖像檢測系統，它整合了三種不同的特徵分析方法（PRNU、ELA、CLIP）來提高檢測的準確性和穩健性。系統採用滑動窗口（Tiling）的方式對任意尺寸的圖像進行全面分析，並透過一個訓練好的融合模型（Logistic Regression）來結合各模型的判斷，最終給出一個綜合評分。
